...
const NAV_ITEMS = [
  ...
  { id: "incidents", label: "Incidents", icon: Globe },
  { id: "top_offenders", label: "Offenders", icon: Crosshair },
  { id: "analytics", label: "Analytics", icon: BarChart3 },
  { id: "ml_playbook", label: "ML", icon: MonitorPlay },
  { id: "signatures", label: "Rules", icon: ShieldCheck },
  { id: "settings", label: "Settings", icon: Settings },
];

export function BottomNavBar({ activeTab, onTabChange, alertCount }: Props) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-md border-t border-white/[0.06] lg:hidden safe-area-bottom" role="navigation" aria-label="Mobile navigation">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-0.5 py-1 px-1.5 rounded-lg transition relative ${
